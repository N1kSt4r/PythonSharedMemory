...
void assert_exc(bool condition, const std::string& message = "") {
    if (not condition) {
...
void from_vector(const std::vector<T, A>& vector, np::ndarray* ndarray) {
    assert_exc(ndarray->get_flags() & np::ndarray::bitflag::C_CONTIGUOUS, "Non contiguous data");
    assert_exc(vector.size() == get_size(*ndarray), "Mismatch data shape");
...
SharedStore<T>::SharedStore(const char* name, size_t size, bool is_server) {
    for (int ntry = 0; ntry < _attempts; ++ntry) {
        {
            ...
            assert_exc(not _inited.count(name), "Double init " + std::string(name) + " store");
    ...
    throw std::runtime_error("Shared store " + std::string(name) + " is not inited in " +
...
void SharedStore<T>::get_dict_meta(py::dict& dict) {
    ...
    assert_exc(get_time() - *_timestamp < _max_timestamp_diff, "Too old last update");
...
typename SharedStore<T>::ShapeType SharedStore<T>::pyshape2cshape(const np::ndarray& array) {
    ...
    assert_exc(dims <= max_dim, "Too many dimensions");
    ShapeType shape;
